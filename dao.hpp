...
class dao_t final {
 public:
  ...
  bool drop_table() {
    ...
    return conn_->execute("drop table if exists " +
  ...
  template <typename T, typename... Args>
  bool get_object(std::vector<T>& v, std::string where_condition = "",
                  Args&&... args) {
    if (!conn_)
      return false;

    v = conn_->template query<T>(std::move(where_condition),
  ...
  static void init(int max_conns, const char* ip, const char* usr,
                   const char* pwd, const char* db_name, int timeout = 1,
                   int port = 3306) {
    ormpp::connection_pool<DB>::instance().init(max_conns, ip, usr, pwd,
                                                db_name, timeout, port);
  }
